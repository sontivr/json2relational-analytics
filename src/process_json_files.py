          ...
          for order in orders:
             ...
             if order['line_items'] is not None:
                for lineitem in order['line_items']:
                   lineitem_plus = {**lineitem, **{'order_id': order['id'], 'order_number': order['order_number'], 'user_id': order['user_id'], 'email': order['email'], 'phone': order['phone'], 'load_ts':self.load_ts}}
   ...
   def load(self):
       ...
       for f in os.listdir(self.output_dir):
          ...
          if re.match(".*orders.csv$", f):
             ...
             cmd = 'cat {} | psql -h {} -p {} -U {} {} -c "COPY myapp.orders from STDIN DELIMITER \',\' NULL \'\' ;\"'.format(orders_csv, self.host, self.port, self.user, self.dbname)

          if re.match(".*line_items.csv$", f):
             line_items_csv = '{}/{}'.format(self.output_dir, f)
             cmd = 'cat {} | psql -h {} -p {} -U {} {} -c \"COPY myapp.line_items from STDIN DELIMITER \',\' NULL\'\' ;\"'.format(line_items_csv, self.host, self.port, self.user, self.dbname)
